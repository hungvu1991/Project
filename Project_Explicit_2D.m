...
for k = 1:Poi
    ...
    U_New(:,1) = (y-ay).^2.*cos(pi*y/ay); %Left Boundary Conditions
    U_New(:,Ny) = y.*(y-ay).^2; %Right Boundary Conditions
    faby = (by-ay)^2*cos(pi*by/ay); %Given for the top boundary condition
    gaby = by*(by-ay)^2; %Given for the top boundary condition
    U_New(1,:)= fliplr(faby+(x-ax)./(bx-ax)*(gaby-faby));  %Top Boundary Conditions
    %%
